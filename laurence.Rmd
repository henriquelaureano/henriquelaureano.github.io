...
x <- 1:25
## 22 days per row
y <- c(0, 0, 0, 1, 1, 2, 1, 1, 1, 1, 1, 3, 2, 0, 1, 0, 0, 0, 1, 1, 2, 1,
       NA, NA, NA)
...
myplot <- function(x, y, model, title, subtitle) {
    ...
    ggplot(data.plot, aes(x=x, y=y))+
        geom_ribbon(aes(xmin=nth(x, -4L), xmax=last(x)), alpha=0.25)+
...
(plot.rw1|plot.ar1|plot.spde)+
    plot_annotation(caption='Gray shadow: 3 days ahead prediction')
```
