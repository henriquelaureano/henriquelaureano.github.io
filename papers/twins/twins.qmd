...
p5 <- dat|>
    ...
    labs(x=NULL, y=NULL, title='Zigosity')+
    theme_classic(base_size=14)+
    theme(axis.text.y=element_text(face='bold', size=14),
          plot.title =element_text(face='bold'))
...
The main used R packages are: `{dplyr}` [@dplyr1.0.9], `{tidyr}`
[@tidyr1.2.0], `{ggplot2}` [@ggplot2.16], `patchwork` [@patchwork1.1.1],
`{kableExtra}` [@kableExtra1.3.4], `{Matrix}` [@matrix1.4.1] and
`{mglm4twin}` [@mglm4twin; @bonat&hjelmborg; @mcglm.seriesc; @mcglm].

# Results

Modelamos as medidas de TREC e KREC conjuntamente em duas frentes: na
média e na covariância. Para estudar a herdabilidade e o relacionamento
genético e ambiental das medidas, usamos um modelo chamado de
ACE. Basicamente, decompomos a matriz de variância-covariância (*just*
matriz de covariância) em três matrizes:
...
```{r}
#| label: AE-bivariate-model-summary

cap <- 'AE bivariate model summary'

rbind(cbind(' '=rownames(tt[[1]][[1]]), tibble::tibble(tt[[1]][[1]])),
      cbind(' '=rownames(tt[[2]][[1]]), tibble::tibble(tt[[2]][[1]])))|>
...
```{r}
#| label: AE-bivariate-model-covariance-components-dispersion

cap <- 'AE bivariate model covariance components / dispersion'
...
```{r}
#| label: AE-bivariate-model-covariance-components-estimates

cap <- 'AE bivariate model covariance components / estimates'
...
```{r}
#| label: AE-bivariate-model-covariance-components-cross

cap <- 'AE bivariate model covariance components / cross'
...
```{r}
#| label: TREC-AE-model-covariance components

cap <- 'TREC AE model covariance components'
...
```{r}
#| label: KREC-AE-model-covariance components

cap <- 'KREC AE model covariance components'
