...
We have the TREC and KREC measures of 100 pairs of twins. From these 200
twins, we have five covariates, as follow:
...
`r round(mean(dat$TREC), 3)` and a median of
`r round(median(dat$TREC), 3)`. For the KREC we have a mean of
`r round(mean(dat$KREC), 3)` and a median of
`r round(median(dat$KREC), 3)`. In the scatterplots we see a clear
