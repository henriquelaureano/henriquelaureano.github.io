...
> interrelações genéticas e ambientais de dados de gêmeos (Bonat and
> Hjelmborg, 2022) foram aplicados para entender a dinâmica das medidas
> de TREc e KREC.

The main used R packages are: `{dplyr}` [@dplyr1.0.9], `{stringr}` [],
`{ggplot2}` [@ggplot2.16], `{tidyr}` [@tidyr1.2.0], `{broom}`
[@broom0.8.0] and `{mglm4twin}` (Bonat, 2018).
