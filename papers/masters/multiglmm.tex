...
data scheme. More specifically, its clustered version i.e., with groups
of elements sharing some non-observed latent dependence structure.
...
differentiation. The main contributions of this article are: (i)
introducing the modeling of cause/cluster-specific CIFs of clustered
...
\begin{itemize}
 ...
 \item Last but not least, the \(w\)'s. With negative values, we have a
   decreasing curve and with positive values an increasing curve i.e.,
   we are interested only on the positive side.
\end{itemize}
...
between \(\eta\) and \(u\) sounds much more realistic, where late onset
...
Broyden-Fletcher-Goldfarb-Shanno (BFGS) algorithm or the PORT routines,
all available in the R statistical software.
