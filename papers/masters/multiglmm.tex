...
standard deviations decrease 50\% or more, if compared with the
...
in the survival modeling literature \citep{kalb&prentice}. We model the
clustered competing risks on a latent-effects framework, a generalized
linear mixed model (GLMM) \citep{GLMM}, with a multinomial distribution
...
\citep{kalb&prentice,andersen12}. The CIF is accommodated in our GLMM
framework in terms of the link function \citep{GLM89}, as the product of
...
fashion. The shape of these functions is described in detail in Section
\ref{model}. This particular GLMM formulation is what makes our model,
particular. Thus, we have what we call a multiGLMM: a multinomial GLMM
for clustered competing risks data.
...
\citep{tierney,patrao} together with an automatic differentiation (AD)
\citep{corestats,nocedal&wright} routine, all via \texttt{TMB}, is able
to efficiently handle with a high number of clusters, independent of its
...
Four latent effects structure configurations were tested, with the
so-called complete model, a model with latent effects on both risk and
failure time trajectory levels with cross-correlations, presenting the
best results. In its biggest scenario, with 60 thousand data points and
clusters of size 2, i.e. 30 thousand four-dimension integral
approximations (ten parameters in the covariance matrix), the model
fitting takes 30 minutes, in parallel, with \texttt{TMB}. Before
\texttt{TMB}, we did a complete R implementation. We wrote the marginal
log-likelihood in \texttt{R}, based on our own Laplace approximation
\citep{patrao} and Newton-Raphson implementation (gradients and Hessian
computed by hand). This complete \R implementation in a scenario with 20
thousand data points and clusters of size 2, took around 30 hours, in
parallel. In summary, by using \texttt{TMB} we were able to increase the
model size 3 times and to decrease the computational time 60 times. An
...
modest scenario with 5 thousand data points and clusters of size 2. It
took around 1 whole week of parallelized processing in system 1. The
results were basically the same as the ones obtained with \texttt{TMB}
but this high computational time just reinforces the, still, MCMC
...
the desired curve behavior. However, in counterpart, its derivatives
w.r.t. time, generates very small probabilities for the failure
competing causes, ending by concentrating almost everything on
censorship, \autoref{fig:modelsample}. For each competing cause with
poor data representativity, we have three curve shape parameters to
estimate, implying the necessity of having a lot of data to then have
enough information about the causes.
