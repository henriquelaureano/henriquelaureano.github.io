...
homeperformance <- function(teams, ploteam, n_rounds, mod) {
    ...
    dat_plot <- cbind(seq(n_rounds), f_mean, f_lower, f_upper)
    names(dat_plot)[1] <- "round"
    dat_plot <- dat_plot %>%
        gather(key = "team", value = "value", -round, -type)

    dat_plot <- as.tibble(dat_plot, .name_repair = "minimal") %>%
        pivot_longer(-round)
...
plot.re_all <- plot.re_rw1 | plot.re_ar1 | plot.re_matern
