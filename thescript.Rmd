...
TeamA | TeamB | match | home-adv | num goals scored by A vs B in match k |

```{r}
dat1 <- data.frame()
for(i in 1:n_matches){
  dat1 = rbind(dat1,c(dat$home[i], dat$away[i], dat$round[i], 1, dat$x[i]),
                c(dat$away[i], dat$home[i], dat$round[i], -1, dat$y[i]))
}
names(dat1) = c("TeamA", "TeamB", "round", "home.adv", "score")
head(dat1)
```
...
$$
	x_i \ \sim \ \text{Poisson}\left(\exp(\mu_H + s^t_{A_i} - s^t_{B_i})\right) \quad \{i=1,3,5,\dots\} \quad \{t=1,\dots,38\}
$$
$$
  \log(\lambda_{x_i})=\mu_H  + s^t_{A_i} - s^t_{B_i}  \quad \{i=1,3,5,\dots\} \quad \{t=1,\dots,38\}
$$
$$
	y_i \ \sim \  \text{Poisson}\left(\exp(-\mu_H  + s^t_{B_i} - s^t_{A_i})\right). \quad \{i=2,4,6,\dots\} \quad \{t=1,\dots,38\}
$$
$$
  \log(\lambda_{y_i})=-\mu_H  + s^t_{B_i} - s^t_{A_i}  \quad \{i=2,4,6,\dots\} \quad \{t=1,\dots,38\}
$$

where the index $t$ again defines the values info related to the variable 'round' in the dataset. For the implementation, we can more or less follow what was done earlier, but keeping in mind that each strength team now depends on the time each match was played during the season. Hence the 'copy 'structure is now being applied on the variable 'round' and 'round2'(of course a copy of the variable round for TeamB \dots) as follow

```{r}
dat4 = dat1 %>% as_tibble() %>% mutate(
  round.home = round,
  round.away = round,
  round = NULL
)
...
models.outputs <- function(data, mod, prior = NULL,  mod.only = FALSE) {
  ...
  plot(1:n_rounds,
       mod.inla$summary.random$round.home$mean[inla.find(
                                              idx = index.top5[1],
                                              n_teams = n_teams)],
       type = 'l', col = 1, ylim = c(0, 1), xlab = 'rounds',
       ylab = 'Posterior mean',
       main = 'Premier League Top5 Performance')
  lines(1:n_rounds,
        mod.inla$summary.random$round.home$mean[inla.find(
                                               idx = index.top5[2],
                                               n_teams = n_teams)],
  ...
  lines(1:n_rounds,
        ...
        col = 3)
  lines(1:n_rounds,
        mod.inla$summary.random$round.home$mean[idx = inla.find(
                                                    index.top5[4],
                                                    n_teams = n_teams)],
        col = 4)
  lines(1:n_rounds,
        mod.inla$summary.random$round.home$mean[idx = inla.find(
                                                    ...
                                                    n_teams = n_teams)],
        col = 5)
  legend("topright", top5, col = 1:5, lty = c(1, 1, 1, 1, 1), cex = 0.8)
}

rw1.prior = list(prec = list(prior = "pc.prec", param = c(1, 0.01)))
M4a.res = models.outputs(data = dat4,
                         mod = 'rw1',
                         prior = rw1.prior, mod.only = TRUE)
models.outputs(data = dat4, mod = 'rw1', prior = rw1.prior)
```

Similar to what we have done for the previous RW2 model we use a PC
prior on both precision $\tau$ and correlation parameter $\rho$ of the
AR1 model respectively given by \texttt{theta1} and \texttt{theta2} in
