...
elpt |> dplyr::filter(
  ...
  dplyr::count(name) |>
  dplyr::mutate(Percentual = n / 73) |> dplyr::arrange(dplyr::desc(n))

```

Dos 73 pacientes com quadro infeccioso, 32 apresentaram em mais de um sistema (ao menos em dois). 43.84%.

Nestes 32, estes são os sintomas que aparecem e em quantos dos 32 pacientes.
...
(
  proxy2 <- 
    ...
    dplyr::mutate(Percentual = n / 32) 
)

```

O máximo de infecções que um paciente apresenta são 5, e 3 pacientes ficaram em tal situação.
...
-Em caso negativo responder com NA`) |> 
  dplyr::mutate(Percentual = n / 114) |>
  dplyr::arrange(dplyr::desc(n)) |> dplyr::filter(n > 1)
  
```

Entre os 114 pacientes com comorbidade, 83 diferentes combinações de comorbidade são observadas - mas apenas 14 aparecem em ao menos dois pacientes.

Mesma coisa mas agora reunindo todos os resultados similares em um único grupo de soma.

55 diferentes, 27 aparecendo ao menos duas vezes.
...
elpt |> 
  ...
  dplyr::select(Comorbidades = `21.1 -  Qual comorbidade o paciente possui?
...
22 pacientes, 20 diferentes combinações de comorbidades - apenas 2 ocorrendo em ao menos dois pacientes.
...
Reunindo os resultados similares em um único grupo, temos 27 diferentes.
...
# Levando em consideração pacientes que fizeram os exames ELPT + IgG, definir um valor de referência para ELPT levando como referência a dosagem de IgG
...
-Responder com NA em caso de não realização do exame`) |>
    ...
    dplyr::filter(IgG_Referência != "NA") |> 
    dplyr::select(IgG, ELPT, IgG_Referência, Age_Group) |>
    tidyr::unnest(IgG)  
...
calc_roc_scenarios <- function(data) {
  ...
  # Generate ROC
  roc_obj <- roc(data$low_igg_flag, data$ELPT, quiet = TRUE)
...
kable(
  ...
  digits = 3, caption = "Optimal ELPT Cutoffs by Age Group"
)
...
age_analysis_data |>
  ...
  scale_fill_manual(values = c("Normal" = "steelblue", "Abaixo do valor de referência" = "orange")) +
  labs(
    title = "ELPT Diagnostic Thresholds by Age Group",
...
## Age-Stratified Analysis: Defining Precision Thresholds for ELPT

Recognizing that immunological profiles evolve with age, we performed a stratified ROC analysis to determine if the optimal ELPT cutoff varies across different pediatric life stages.

### Methodology

The dataset was segmented into three age cohorts: **1 to 3 years**, **4 to 9 years**, and **10 to 17 years**. For each group, we calculated the optimal "Balanced" threshold (Youden's Index) to maximize diagnostic accuracy.

### Results by Age Cohort

The analysis revealed a clear distinction between pre-pubertal children and adolescents:

1.  **Early Childhood (1 to 3 years):**
    -   **Optimal Cutoff:** **0.375**
    -   **Performance:** The test performs robustly, achieving **90.0% Sensitivity** and **95.8% Specificity**.
    -   *Interpretation:* The clear separation between groups allows for high-confidence screening in toddlers.
2.  **Middle Childhood (4 to 9 years):**
    -   **Optimal Cutoff:** **0.380**
    -   **Performance:** In this cohort, the model achieved perfect classification (**100% Sensitivity and Specificity**).
    -   *Interpretation:* The optimal threshold (0.380) is virtually identical to the toddler group (0.375), suggesting a highly stable physiological baseline for ELPT throughout the first decade of life.
3.  **Adolescence (10 to 17 years):**
    -   **Optimal Cutoff:** **0.475**
    -   **Performance:** Diagnostic difficulty increases significantly. Sensitivity drops to **53.3%** and Specificity to **75.0%**.
    -   *Interpretation:* The optimal cutoff shifts upward by nearly 0.1 g/dL compared to younger children. The significant distributional overlap (visible in the density plots) suggests that ELPT becomes a less binary marker during adolescent immune maturation, requiring more careful clinical interpretation.

### Clinical Implication

The results support a **Two-Tiered Reference Approach**:

-   **For Children \< 10 Years:** A unified cutoff of **0.38 g/dL** is recommended. This threshold is stable and highly accurate across both the 1-3 and 4-9 age groups.
-   **For Adolescents (10-17 Years):** A higher cutoff of **0.475 g/dL** is indicated. However, clinicians should be advised that ELPT has lower sensitivity in this age group, and borderline results should almost always trigger confirmatory IgG testing.

```{r warning=FALSE, fig.width=9.75, fig.height=4.5}

# 1. Define Age-Specific Cutoffs (Based on your previous ROC results)
# We create a small lookup table to map each age group to its Youden cutoff
cutoff_map <- tibble(
  Age_Group = c("1 ano a 3 anos", "4 anos a 9 anos", "10 anos a 17 anos"),
  Age_Specific_Cutoff = c(0.375, 0.380, 0.475)
)

# 2. Prepare Data & Apply Cutoffs
infection_analysis_age <- elpt |>
  dplyr::rename(
    ELPT = `16 - Resultado do Primeiro exame`,
    Age_Group = `3 - Insira Faixa etária do paciente`
  ) |>
  # Filter to keep only the groups we analyzed
  ...
  dplyr::mutate(Risk_Group = ifelse(ELPT < Age_Specific_Cutoff, "High_Risk", "Low_Risk")) |>
  ...
  # Pivot Longer
  tidyr::pivot_longer(
    cols = matches("^(8|9|10|11|12|13|14|15) -"), # Selects all infection columns
    names_to = "Infection_Type",
    values_to = "Status"
  ) |>
...
# Display Table
kable(final_results_age, digits = 2, caption = "Impact of Low ELPT on Infections (Stratified by Age)")
...
final_results_age |>
  ...
  scale_color_manual(values = c("Yes" = "#D55E00", "No" = "gray60")) +
  labs(
    title = "ELPT Infection Risk by Age Cohort",
...
To ensure biological accuracy, we assessed the clinical impact of ELPT deficiency using the specific thresholds derived for each age cohort (**0.375** for 1-3y, **0.380** for 4-9y, and **0.475** for 10-17y).

-   **Methodology:** Patients were stratified by age, and "High Risk" status was defined individually for each group based on their specific optimal cutoff. We compared infection prevalence rates between the "High Risk" (Low ELPT) and "Low Risk" (Normal ELPT) groups.

-   **Results by Cohort:**

    1.  **Adolescents (10 to 17 years): A Distinct Clinical Signal**
        -   **Lower Respiratory Vulnerability:** This age group presented the only statistically significant finding in the analysis ($p = 0.03$).
        -   **The Data:** **50.0%** of adolescents in the "High Risk" group suffered from Lower Respiratory Infections (e.g., Pneumonia/Bronchitis), compared to **0.0%** in the "Low Risk" group.
        -   **Implication:** For adolescents, an ELPT value below 0.475 is a strong specific predictor for lower respiratory susceptibility.
    2.  **Early Childhood (1 to 3 years): Clinical Trends vs. Statistical Power**
        -   **Sepsis:** A clinically relevant trend was observed where "High Risk" toddlers had a higher rate of Sepsis (**42.86%**) compared to the "Low Risk" group (**27.59%**). However, due to sample size limitations, this did not reach statistical significance ($p = 0.65$).
        -   **Urinary Infections:** Similarly, urinary infections were more frequent in the risk group (42.86% vs 24.14%), though statistically non-significant ($p = 0.37$).
    3.  **Middle Childhood (4 to 9 years):**
        -   **No Clear Association:** In this intermediate age range, infection rates were variable, with no clear pattern identifying the "High Risk" group as more susceptible to specific infections. For example, lower respiratory infections were actually more common in the "Low Risk" group for this specific cohort.

**Conclusion:** The age-stratified analysis demonstrates that the clinical utility of ELPT varies by developmental stage. While it serves as a broad biochemical screen for all ages, it functions as a **phenotypic predictor for lower respiratory infections specifically in adolescents**. In toddlers (1-3 years), it may flag a general susceptibility to systemic (Sepsis) and urinary infections, though larger sample sizes would be required to confirm this statistically.

# Medicações vs ELPT e IgG

```{r}

elpt |> 
  dplyr::count(`34 - Medicações Utilizadas`) |>
  dplyr::mutate(Percentual = n / nrow(elpt)) |> dplyr::arrange(dplyr::desc(n))
...
Medidas descritivas de ELPT e IgG pelo uso de medicação.
...
Graficamente os valores de ELPT não parecerem diferir muito pelo fato do uso de medicação, mas o paciente outlier no valor de IgG distorce muito a visualização.
...
-Responder com NA em caso de não realização do exame`
  ...
  tidyr::drop_na(IgG) |>
  ggplot() +
  aes(x = IgG, y = ELPT, fill = Medicação, color = Medicação) +
  geom_point(size = 3, shape = 21, alpha = 0.6) +
  geom_smooth(method = "lm", 
              formula = y ~ poly(x, 1), 
              color = "red", 
...
p2 <- elpt |>
  ...
  ggplot() +
  aes(x = ELPT, fill = Medicação) +
  geom_histogram(alpha = 0.8, binwidth = 0.025) +
  labs(title = "Histograma de ELPT",
       subtitle = "Binwidth: 0.025",
       fill = "Medicação: ",
       x = "ELPT (g/dL)", y = "n") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.1)) +
...
Ajustamos também um modelo linear para entender melhor as associações com o uso de medicação - após verificar os resíduos e constatar que o ajuste estava adequado, performamos uma Análise de variância (ANOVA) e com base no teste F obtivemos:

-   um valor-p de 0.3649, indicando que realmente não existe associação entre o valor de IgG e o fato de utilizar ou não um medicamento. Apesar da presençã do outlier, mesmo assim a associação não é significativa.

-   Para ELPT obtivemos um valor-p de 0.08675, pelo critério de 5%, não se trata duma associação estatísticamente significativa entre o valor de ELPT e o fato de utilizar ou não um medicamento.
...
> Paciente apresenta perda proteica na urina nos 30 dias que antecederam a realização do exame ELPT?

```{r}

elpt |>
  dplyr::select(
    PPU=`25 - Paciente apresenta perda proteica na urina nos 30 dias que antecederam a realização do exame ELPT?`
  ) |>
  dplyr::count(PPU) |>
  dplyr::mutate(Percentual = n / nrow(elpt))
...
Tanto pelas medidas quanto visualmente não aparenta ter diferença significativa tanto de ELPT quanto de IgG pelo fato de ter tido perda proteica na urina ou não - apesar do outlier.
...
-Responder com NA em caso de não realização do exame`
  ...
  tidyr::drop_na(IgG) |>
  ggplot() +
  aes(x = IgG, y = ELPT, fill = PPU, color = PPU) +
  geom_point(size = 3, shape = 21, alpha = 0.6) +
  geom_smooth(method = "lm", 
              formula = y ~ poly(x, 1), 
              color = "red", 
...
Ajustamos também um modelo linear para entender melhor as associações com a perda proteica de urina - após verificar os resíduos e constatar que o ajuste estava adequado, performamos uma Análise de variância (ANOVA) e com base no teste F obtivemos:

-   um valor-p de 0.3748, indicando que realmente não existe associação entre o valor de IgG e a perda proteica de urina.

-   Para ELPT obtivemos um valor-p de 0.04959, uma associação no limite de ser considerada estatísticamente significativa - significatica mas fraca, levando em consideração também o desbalanceamento de classes aqui com apenas 15% dos pacientes tendo perda proteica de urina.
...
>
> Garrett Grolemund, Hadley Wickham (2011). *Dates and Times Made Easy with lubridate*. Journal of Statistical Software, 40(3), 1-25. <!-- > --> <!-- > McCullagh, P., & Nelder, J. A. (1989). *Generalized Linear Models* (2nd ed.). Chapman & Hall. --> <!-- > --> <!-- > Hastie, T., & Tibshirani, R. (1990). *Generalized Additive Models*. Chapman & Hall. --> <!-- > --> <!-- > Wood, S. N. (2017). *Generalized Additive Models: An Introduction with R* (2nd ed.). CRC Press. -->
