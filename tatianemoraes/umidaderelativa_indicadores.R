...
##                                      2022-mai-11 · Curitiba/PR/Brazil
##----------------------------------------------------------------------

## ---------------------------------------------------------------------
## Indicadores de malária: Umidade Relativa ----------------------------
## ---------------------------------------------------------------------
...
dat1.median <- indicator(urelativa, dat1.vars, index='median')
dat1.mean   <- indicator(urelativa, dat1.vars, index='mean')
...
urelativa.br <- dplyr::inner_join(br, urelativa, by='code_muni')

rbind(morans(urelativa.br, urelativa.br$`Presença de rodovias`))|>
    kableExtra::kbl(booktabs=TRUE, digits=10)|>
    kableExtra::kable_classic_2(full_width=FALSE)|>
...
dat3.mean   <- indicator(urelativa, dat3.vars, index='mean')
dat3.pca    <- indicator(urelativa, dat3.vars, index='pca')
...
dat4.median <- indicator(urelativa, dat4.vars, index='median')
dat4.mean   <- indicator(urelativa, dat4.vars, index='mean')
...
dat5.mean   <- indicator(urelativa, dat5.vars, index='mean')
dat5.pca    <- indicator(urelativa, dat5.vars, index='pca')
...
dat6.mean   <- indicator(urelativa, dat6.vars, index='mean')
dat6.pca    <- indicator(urelativa, dat6.vars, index='pca')
...
dat9 <- tibble::tibble(ind1=urelativa$`Presença de rodovias`, 
                       ind2=dat1.pca$index)
...
indicadores <-
    tibble::tibble(
                ...
                'Malha rodoviária'=urelativa$`Presença de rodovias`,
                ...
                'Índice de Risco de Impacto para umidade relativa (média)'=dat11.mean$index, 
                'Índice de Risco de Impacto para umidade relativa (mediana)'=dat11.median$index, 
                'Índice de Risco de Impacto para umidade relativa (PCA)'=dat11.pca$index
            )
write.csv(indicadores, 'indicadores_umidaderelativa.csv', row.names=FALSE)
