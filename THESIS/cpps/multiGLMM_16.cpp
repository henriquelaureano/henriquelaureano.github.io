...
// DATE: 11/18/2020 (MONTH / DAY / YEAR)
#include <TMB.hpp>
template<class Type>
Type objective_function<Type>::operator() ()
{
  ...
  Sigma.row(0) <<
    exp(logs2(0)), rho(0) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(1))),
    0, 0;
  Sigma.row(1) <<
    rho(0) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(1))), exp(logs2(1)),
    0, 0;
  Sigma.row(2) << 0, 0,
    exp(logs2(2)), rho(1) * sqrt(exp(logs2(2))) * sqrt(exp(logs2(3)));
  Sigma.row(3) << 0, 0,
    rho(1) * sqrt(exp(logs2(2))) * sqrt(exp(logs2(3))), exp(logs2(3));
  // -------------------------------------------------------------------
