...
## TAKE-HOME MESSAGE ---------------------------------------------------
## DO ALL INITIAL GUESSES LEAD TO THE SAME PLACE? YES
...
y <- datasimu(J=J, cs=cs, time=time,
              ...
              latent='partial', type='trajectory', Sigma=Sigma)
prop.table(colSums(y))

## ~/Git/henriquelaureano.github.io/THESIS/code/
dll <- 'v2';invisible(TMB::compile(paste0('cpps/', dll, '.cpp')))
