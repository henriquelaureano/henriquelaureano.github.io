...
// DATE: 01/19/2021 (MONTH / DAY / YEAR)
#include <TMB.hpp>
template<class Type>
Type objective_function<Type>::operator() ()
{
  ...
  ps.col(2) = p3;
  // -------------------------------------------------------------------
  vector<Type> rho = (exp(2 * rhoZ) - 1)/(exp(2 * rhoZ) + 1);
  matrix<Type> Sigma(4, 4);
  Sigma.row(0) <<
    exp(logs2(0)),
    rho(0) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(1))),
    rho(2) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(2))),
    rho(4) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(3)));
  Sigma.row(1) <<
    rho(0) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(1))),
    exp(logs2(1)),
    rho(5) * sqrt(exp(logs2(1))) * sqrt(exp(logs2(2))),
    rho(3) * sqrt(exp(logs2(1))) * sqrt(exp(logs2(3)));
  Sigma.row(2) <<
    rho(2) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(2))),
    rho(5) * sqrt(exp(logs2(1))) * sqrt(exp(logs2(2))),
    exp(logs2(2)),
    rho(1) * sqrt(exp(logs2(2))) * sqrt(exp(logs2(3)));
  Sigma.row(3) <<
    rho(4) * sqrt(exp(logs2(0))) * sqrt(exp(logs2(3))),
    rho(3) * sqrt(exp(logs2(1))) * sqrt(exp(logs2(3))),
    rho(1) * sqrt(exp(logs2(2))) * sqrt(exp(logs2(3))),
