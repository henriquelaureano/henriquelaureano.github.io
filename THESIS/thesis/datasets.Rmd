...
dat  <- datasimu(J=J, cs=cs, time=time, Z=Z, S=S)
```
...
dat_nocens <- dat_longy %>% filter(y_label != 'y3') %>%
    mutate(y_label=forcats::fct_recode(y_label,
                                       'Failure by cause 1'='y1',
                                       'Failure by cause 2'='y2'))

ggplot(dat_nocens, aes(x=time))+
...
ggplot(dat_longp, aes(x=p_value))+
    ...
    geom_label(data=data.frame(
               ...
               mapping=aes(x=c(0.0625, 0.052, 0.9375), y=Inf, label=label),
               vjust=1.5)+
    theme_minimal()+
