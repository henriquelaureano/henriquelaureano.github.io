...
```{r readdata}
path       <- '~/Git/henriquelaureano.github.io/THESIS/code/coefs/'
...
for (i in 1:3)
{
    for (j in 1:2)
    {
        for (k in 1:3)
        {
            ...
            v1data[[l]] <- vdata(paste0('coefsv1_', label[l], '.txt'))
            v2data[[l]] <- vdata(paste0('coefsv2_', label[l], '.txt'))
...
v1true <- rep(
    c(replicate(v1cif1, n=3, simplify=FALSE),
      replicate(v1cif2, n=3, simplify=FALSE)),
    3
)
v2cif1 <- c(cif1, logvars[3:4], rhoZs[6])
v2cif2 <- c(cif2, logvars[3:4], rhoZs[6])
v2true <- rep(
    c(replicate(v2cif1, n=3, simplify=FALSE),
      replicate(v2cif2, n=3, simplify=FALSE)),
    3
)
v3cif1 <- c(cif1, logvars, rhoZs[c(1, 6)])
v3cif2 <- c(cif2, logvars, rhoZs[c(1, 6)])
v3true <- rep(
    c(replicate(v3cif1, n=3, simplify=FALSE),
      replicate(v3cif2, n=3, simplify=FALSE)),
    3
)
v4cif1 <- c(cif1, logvars, rhoZs)
v4cif2 <- c(cif2, logvars, rhoZs)
v4true <- rep(
    c(replicate(v4cif1, n=3, simplify=FALSE),
      replicate(v4cif2, n=3, simplify=FALSE)),
    3
)
```
...
bias2plot
func(data, title, zrhos)

biasformat <- function(errordata)
{
    out <-
        errordata%>%
        dplyr::select(beta1, label, conv, nr, data)%>%
        pivot_wider(names_from=label, values_from=beta1)
    out$data <- forcats::fct_relevel(forcats::as_factor(out$data), rev)
    return(out)
}

df <- dplyr::bind_rows(biasformat(v1error),
                       biasformat(v2error),
                       biasformat(v3error),
                       biasformat(v4error),
                       .id='v')
...
```{r cor2plot,message=FALSE,fig.height=10,fig.width=10}
