...
```{r biasaqua,fig.width=10,fig.height=6.5}
bias2plotAqua <- function(df, title)
{
    ggplot(df, aes(ymin=mean-1.96*sd, ymax=mean+1.96*sd, x=data, size=nr))+
        geom_linerange(position=position_dodge(width=0.2), color='#0096c8')+
        geom_point(mapping=aes(x=data, y=mean), size=3,
...
bias2aqua <- function(par, title)
{
    out <- biasformat(
        par,
        list(v1error, v2error, v3error, v4error)
    )
    bias2plotAqua(out, title=title)
}
bias2aqua('beta1', expression(bold('Parameter:'~beta[1])))
...
out$v <- forcats::fct_recode(out$v,
                             ...
                             'BLOCK-DIAG MODEL'='2',
                             'COMPLETE MODEL'  ='3')
bias2plotAqua(out, expression(bold('Parameter:'~log(sigma[4]^2))))
...
out$v <- forcats::fct_recode(out$v,
                             ...
                             'BLOCK-DIAG MODEL'='2',
                             'COMPLETE MODEL'  ='3')
bias2plotAqua(out, expression(bold('Parameter:'~z(rho[12]))))
...
out$v <- forcats::fct_recode(out$v,
                             ...
                             'BLOCK-DIAG MODEL'='2',
                             'COMPLETE MODEL'  ='3')
bias2plotAqua(out, expression(bold('Parameter:'~z(rho[34]))))

bias2aqua_rho <- function(df, title)
{
    ggplot(df, aes(ymin=mean-1.96*sd, ymax=mean+1.96*sd, x=data, size=nr))+
        geom_linerange(position=position_dodge(width=0.2), color='#0096c8')+
        geom_point(mapping=aes(x=data, y=mean), size=3,
...
```{r cifsaqua,fig.width=10,fig.height=7.5}
p5 <-
    p1+
...
```{r histologs2,fig.width=10,fig.height=14}
...
```{r historhoz,fig.width=10,fig.height=10.5}
crossco <- function()
{
    out <- NULL
    for (i in seq(18))
