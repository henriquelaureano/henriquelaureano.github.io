...
worst. For the time model,in the scenario with clusters of size 2, high
CIF, and 5 thousand data points, we get a much bigger standard deviation
in the \(\bm{\beta}\)s parameter estimates. For the block-diag model, in
the scenario with clusters of size 2, low CIF, and 5 thousand data
points, the standard deviations are huge for the curve shape parameter
estimates of the competing cause 1. In the \autoref{cap:appendixD}, with
the 2.5 and 97.5\% bias quantiles, the most extreme values are removed
from the uncertainty representation. There, the main characteristic is
the parameter estimates asymmetry.
