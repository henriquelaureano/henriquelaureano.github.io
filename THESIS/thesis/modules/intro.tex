...
the cause-specific CIF, accouting for an appropriate latent effects
...
to model its mean, \(\mu_{i} \equiv \mathbb{E}(Y_{i})\), via a linear
...
received the name of generalized linear model (GLM) and a comprehensive
reference is \citeonline{GLM89}.
...
mixed model (multiGLMM) to the cause-specific cumulative incidence
function (CIF) of clustered competing risks data.
