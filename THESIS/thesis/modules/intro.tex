...
(explanatory/independent variables) vector \(\mathbf{x}_{i} =
...
\]
where \(\mathbf{x}_{i}\) is the \(i^\text{th}\) row of a model matrix
\(\mathbf{X}\), and \(\bm{\beta}\) is a vector of unknown parameters.
...
\[
  g(\mu_{i}) = \mathbf{x}_{i}\bm{\beta} + \mathbf{z}_{i}\mathbf{u},
