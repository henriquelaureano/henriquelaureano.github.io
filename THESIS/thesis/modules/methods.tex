...
binomial GLMM with a logistic link function.

The latent structure is in the context of repeated measures, the same
subject is observed three times. Trying to keep it simple, no
covariates. A hierarchical model's description is given by
\begin{align*}
 y_{ij} \mid u_{i} &\sim \text{Binomial}(n, p_{ij})\\
            u_{i} &\sim \text{Normal}(0, 1)\\
 g(p_{ij}) = \text{logit}(p_{ij}) &= \beta + u_{i}\\
...
\begin{figure}[H]
 ...
 \caption{EX1}
 \vspace{0.3cm}\centering
 \lstinputlisting[lastline=45]{modules/exampleTMB.R}
 \begin{footnotesize}
...
\begin{figure}[H]
 ...
 \caption{EX2}
 \vspace{0.3cm}\centering
 \lstinputlisting[firstline=47]{modules/exampleTMB.R}
 \begin{footnotesize}
  SOURCE: The author~(2021).
 \end{footnotesize}
 \label{fig:tmbex-p2}
