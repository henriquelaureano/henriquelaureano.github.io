...
\begin{figure}[H]
 ...
 \caption{ILLUSTRATION OF COEFFICIENT BEHAVIORS FOR A GIVEN CUMULATIVE
          INCIDENCE FUNCTION (CIF), PROPOSED BY \citeonline{SCHEIKE}, IN
          A MODEL WITH TWO COMPETING CAUSES OF FAILURE, WITHOUT
          COVARIATES AND THE FOLLOWING CONFIGURATION: \(\beta_{2} = 0\),
          \(\bm{u} = 0\) AND \(\bm{\eta} = 0\); IN EACH SCENARIO ALL
          OTHER COEFFICIENTS ARE SET AT ZERO, WITH THE EXCEPTION OF
          \(w_{1} = 1\)}
...
\begin{figure}[H]
 ...
 \caption{ILLUSTRATION OF A GIVEN CLUSTER-SPECIFIC CUMULATIVE INCIDENCE
          ...
          FOLLOWING CONFIGURATION: \(\beta_{1} = -2\),
