...
@
         ...
         Now, we obtain the likelihood profiles for each parameter. The profiles
         are presented in Figure~\ref{fig:profiles-norm}.
         ...
         likelihood. We do this below for \(\mu\) and \(\sigma\), the results
         can be seen in Figure~\ref{fig:profiles-norm}.
...
(par.est <- bbmle::mle2(lkl.model.norm,
                        start = list(b0 = min(data$y),
                                     b1 = 1.5,
                                     sigma = sd(data$y)),
                        data = list(y = data$y, x = data$x),
                        method = "BFGS"))
# </r code> -----
@
         Just checking...
...
<<dev.model.norm, fig.height=4, fig.width=6, fig.cap="Deviance of (\\(\\beta_{0}, \\beta_{1}\\)) with \\(\\sigma\\) fixed in it's estimative, for a Gaussian linear model.">>=
# <r code> -----
# deviance -----------------------------------------------------------
...
<<lkl.model.norm-profile, fig.height=3.5, fig.width=5, fig.cap="log-likelihood profile for \\(\\sigma\\), for a Gaussian linear model. In dashed, the respective MLE.">>=
# <r code> -----
# profile ------------------------------------------------------------
...
# </r code> -----
@
           Very few, as expected. Since the estimation of \(\beta\) doesn't
           depend on \(\sigma\).
    \end{enumerate}
   \item Obtenha a verossimilhan\c{c}a perfilhada para os par\^{a}metros
         \(\beta_{0}\) e \(\beta_{1}\) individualmente.
<<lkl.model.norm-beta.profiles, fig.height=3.5, fig.cap="log-likelihood profiles for \\(\\beta_{0}\\) and \\(\\beta_{1}\\) for a Gaussian linear model. In dashed, MLE's.">>=
# <r code> -----
par(mfrow = c(1, 2), mar = c(4, 4, 2, 2) + .1) # cosmetics
# beta_0 profile ---------------------------------------------------------
b0.grid <- seq(67.5, 88.5, length.out = 100)
