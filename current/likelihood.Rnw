 ...
 \item A fim de se obter uma estimativa do p\'{u}blico de um jogo sem utilizar
  ...
  \begin{enumerate}[a)]
   \item Obtenha a fun\c{c}\~{a}o de verossimilhan\c{c}a para o n\'{u}mero total
         ...
         We have here a random variable, let's say, \(X\), represeting the
         ...
         The likelihood for the total number of fans is the product from 1 to
         \(n = 250\),
         
         \begin{align*}
          L(N) &= \prod_{i=1}^{250}
                  \text{constant} \frac{(N-300)!~(N-250)!}{(N-538)!~N!}\\
          L(N) &= \text{constant}^{250}
                  \left[\frac{(N-300)!~(N-250)!}{(N-538)!~N!}\right]^{250}\\
          l(N) = \log L(N) &= 250\log~\text{constant} +
                              250\log~\frac{(N-300)!~(N-250)!}{(N-538)!~N!}\\
                           &\approx
                           250\log~\frac{(N-300)!~(N-250)!}{(N-538)!~N!}.
         \end{align*}
