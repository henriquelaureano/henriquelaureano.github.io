...
\pagestyle{fancy}
\title{Exercises about likelihood}
\date{June, 2019}
...
\textbf{Seja \(X\) uma v.a. com distribui\c{c}\~{a}o uniforme
        \(X \sim U(0, \theta)\).\\
        Uma amostra aleat\'{o}ria forneceu os seguintes valores:
        5,5; 3,2; 4;8, 5,3; 3,8 e 5,0. Obtenha a fun\c{c}\~{a}o de
        verossimilhan\c{c}a e um (ou mais) intervalo(s) adequado(s)
        especificando a forma de obten\c{c}\~{a}o.
       }
<<>>=
# <r code> -----
...
Given the data above, we get \(x_{(n)} = \Sexpr{max(data)}\) and the likelihood
is shown in Figure~\ref{fig:uniform}, together with some intervals.
...
\(\theta\), one corresponding to a 95\% confidence interval (dashed line) and
one corresponding to a 99\% confidence interval for the mean (dotted line). In
the left we have a probability-based likelihood interval, in the right we have a
pure likelihood interval.

The left ones are bad intervals, since they're based in a large-sample theor
that results in an exact interval for the Gaussian case, in a good approximation
for a reasonably regular case, and as we can see by the figure... here we
doesn't have a regular likelihood (the likelihood isn't well approximated by a
quadratic function), thus, here this interval should not be so good.
...
points the cutoff horizontal line cuts the likelihood. For this purpose we use
the function \texttt{rootSolve::uniroot.all}.
...
\textbf{Seja \(X\) uma v.a. de uma distribui\c{c}\~{a}o de Poisson
...
\begin{enumerate}[(a)]
 \item \textbf{Obtenha a fun\c{c}\~{a}o de verossimilhan\c{c}a, sua
               aproxima\c{c}\~{a}o quadr\'{a}tica e intervalos para
               \(\lambda\).}
...
approximation around the maximum likelihood estimator (MLE). Here the sample
size is very small, the idea is that as the sample size increase the quality, in
this case the range, of the approximation also increase. This should happen
because as the sample size increase the Poisson likelihood should be more
symmetric.
...
\item \textbf{Repita a quest\~{a}o anterior para a reparametriza\c{c}\~{a}o
              \(\theta = \log (\lambda)\).}
...
on the topright of Figure~\ref{fig:poisson-reparametrization}, and one based in
a cut in the quadratic approximation of the likelihood,
\(\theta \in (\Sexpr{paste(round(wald.95, 3), collapse = ", ")})\) - in red on
...
than the ones obtained for \(\lambda\). i.e., for \(\theta\) (with the use of
the \(\log\)) we get a more regular likelihood.

\item \textbf{Obtenha ainda (por pelo menos dois m\'{e}todos diferentes)
              intervalos de confian\c{c}a para o par\^{a}metro \(\lambda\) a
              partir da fun\c{c}\~{a}o de verossimilhan\c{c}a (aproximada ou
              n\~{a}o) de \(\theta\).}
...
\textbf{A fim de se obter uma estimativa do p\'{u}blico de um jogo sem utilizar
...
\begin{enumerate}[a)]
 \item \textbf{Obtenha a fun\c{c}\~{a}o de verossimilhan\c{c}a para o n\'{u}mero
               total de torcedores.}
...
\item \textbf{Obtenha a estimativa pontual e a intervalar, esta \'{u}ltima por
              pelo menos dois m\'{e}todos diferentes.}
...
(log-)likelihood followed by two intervals, one based in the likelihood, and a
wald interval based in the quadratic approximation. All this is presented in
...
\item \textbf{Repita e compare os resultados caso fossem 500 camisas e 20 com
              camisas dentre os 250.}
...
\section{Normal distribution with interval data}

\textbf{Foram tomadas as seguintes observa\c{c}\~{o}es independentes de uma v.a.
        \(X \sim N(\mu, \sigma^{2})\).}
...
# </r code> -----
@
 \item \textbf{sabe-se que outras 5 observa\c{c}\~{o}es s\~{a}o menores que 50.}
 \item \textbf{sabe-se que outras 3 observa\c{c}\~{o}es s\~{a}o maiores que 65.}
\end{itemize}
  
\begin{enumerate}[(a)]
 \item \textbf{Escrever a fun\c{c}\~{a}o de verossimilhan\c{c}a.}
...
\item \textbf{Obter as estimativas de m\'{a}xima verossimilhan\c{c}a.}
...
\item \textbf{Obter as verossimilhan\c{c}as perfilhadas.}
...
parameters. Second, in the log parametrization of sigma we have better behavior
in the surface.
         
Now, we obtain the likelihood profiles for each parameter. The profiles are
presented in Figure~\ref{fig:profiles-norm}.
         
To obtain the likelihood profile for \(\mu\), let's say, we need to create a
grid of values of \(\mu\) and for each value of this grid find the value, let's
say, \(\hat{\sigma}_{\mu}\) that maximizes the profile likelihood. We do this
below for \(\mu\) and \(\sigma\), the results can be seen in
Figure~\ref{fig:profiles-norm}.
...
\item \textbf{Obter os erro-padr\~{a}o das estimativas.}
...
\textbf{Considere os dados da tabela a seguir (adaptados/modificados de
...
\item \textbf{Encontre a fun\c{c}\~{a}o de verossimilhan\c{c}a.}

<<norm_5-disp, fig.height=3.5, fig.width=5, fig.cap="Dispersion between level of hydrocarbonates \\(\\times\\) purity (\\%).">>=
# <r code> -----
data <- data.frame(
...
par(mar = c(4, 4, 2, 2) + .1)
plot(y ~ x, data,
     xlab = "level of hydrocarbonates", ylab = expression("purity"~("%")))
# </r code> -----
@
...
\item \textbf{Encontre as estimativas de m\'{a}xima verossimilhan\c{c}a.}
...
\item \textbf{Obtenha a verossimilhan\c{c}a conjunta para os par\^{a}metros
              \(\beta_{0}\) e \(\beta_{1}\):}

\begin{enumerate}[i.]

\item \textbf{considerando \(\sigma\) fixo com valor igual \`{a} sua
              estimativa.}
...
\item \textbf{obtendo a verossimilhan\c{c}a (conjunta - 2D) perfilhada em
...
\item \textbf{Obtenha a verossimilhan\c{c}a perfilhada para os par\^{a}metros
...
# </r code> -----
@
   \item Obtenha os perfis de verossimilhan\c{c}a dos par\^{a}metros.
...
pl.b0 <- function(grid) {
    ...
    p.lkl <- numeric(n)
    for (i in 1:n) {
        p.lkl[i] <- mle2(lkl.model.poi, start = list(b1 = .5),
...
@
         ...
         intervals match. A brief, but enough, description is already given
         in the Figure caption, and for more details, we have the code above.
...
# parameter estimates ----------------------------------------------------
model.glm$coef # at three decimal cases, the estimates are equal
round(model.glm$coef, 3) == round(par.est@coef, 3)
# maximum log-likelihood -------------------------------------------------
logLik(model.glm) # at eight decimal cases, the max. log-likes are equal
