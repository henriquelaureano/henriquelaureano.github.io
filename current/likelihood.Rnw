...
@
  \begin{enumerate}[(a)]
   \item Obtenha a fun\c{c}\~{a}o de verossimilhan\c{c}a, sua
         ...
         Computing the Score (derivative of \(l(\lambda)\) wrt to \(\lambda\))
         and making it equal to zero, we find the MLE
         \(\hat{\lambda} = \bar{y}\).\\
         Computing the second derivative we find the observed information
         
         \[
          I_{O}(\lambda) = \frac{n \bar{y}}{\lambda^{2}}
          \quad \rightarrow \quad
          I_{O}(\hat{\lambda}) = \frac{n}{\bar{y}}.
         \]
         
         The Poisson log-likelihood graph is provided in black in the
         ...
         is provided in red in the Figure~\ref{fig:poisson}. There we can see
         how good is the approximation around the maximum likelihood estimator
...
<<poisson, fig.height=5.5, fig.cap="log-likelihood function and MLE of \\(\\lambda\\) in Poisson(\\(\\lambda\\)) based on \\texttt{data}. In the topleft, quadratic approximation in red. In the topright, two intervals for \\(\\lambda\\) - one based in the likelihood (in black) and one based in the quadratic approximation (in red). In the bottom a better look into the quadratic approximation.">>=
# <r code> -----
# likelihood -------------------------------------------------------------
...
@
         ...
         but following a cutoff criterion based in a \(\chi^{2}\) distribution.
         Thus, to have a 95\% confidence interval we use a cutoff \(c = 15\%\).
         As we saw before, this interval is based in a large-sample theory that
         here is a good approximation, since we're dealing with a reasonable
         regular case.
         
         In red we have a interval based in the quadratic approximation of the
         log-likelihood,
         ...
         divided by its variance) and squared.
         
         From this we get the following exact (in the normal case) 95\%
         confidence interval
         ...
         interval (a \(\pm\) interval) is a cut in the quadratic approximation
         exactly in the same point that the probability-based interval cuts the
         (log-)likelihood.
