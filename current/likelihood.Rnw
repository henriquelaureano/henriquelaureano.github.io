...
Using the same functions created before, we first show, in
Figure~\ref{fig:poi_focus-context}, the profiles likelihood together with its
respective quadratic approximations,  providing in this way a better
contextualization to what we'll do next.

<<poi_focus-context, fig.height=5.5, fig.cap="log-likelihood profiles, in black, for \\(\\beta_{0}\\) and \\(\\beta_{1}\\) for a Poisson regression. In black dashed, the MLE. In red, the quadratic approximations of the profiles.">>=
# <r code> -----
par(mfrow = c(2, 1), mar = c(4, 4, 2, 2) + .1) # cosmetics
# beta_0 ---------------------------------------------------------------------
## first, plotting the profiles with point estimate --------------------------
plot(b0.grid, p.b0, type = "l",
     xlab = expression(beta[0]), ylab = "log-likelihood")
abline(v = par.est@coef[1], lty = 2)
## now, the quadratic approximations of the profiles -------------------------
lines(b0.grid, app2.b0.poi, col = 2)
# beta_1 ---------------------------------------------------------------------
plot(b1.grid, p.b1, type = "l",
     xlab = expression(beta[1]), ylab = "log-likelihood")
abline(v = par.est@coef[2], lty = 2)

lines(b1.grid, app2.b1.poi, col = 2)
# </r code> -----
@
